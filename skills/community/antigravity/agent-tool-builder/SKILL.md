...
def get_weather(location: str) -> ToolResult:
    ...
    try:
        ...
        return ToolResult(
            success=True,
            content=f"Temperature: {data.temp}øF, Conditions: {data.conditions}"
        )
...
def get_weather(location: str, unit: str = "fahrenheit") -> str:
    ...
    '''
    # Implementation
    return json.dumps({"temperature": "72øF", "conditions": "Sunny"})
...
const getWeatherTool = betaZodTool({
  ...
  run: async (input) => {
    // Type-safe input!
    return JSON.stringify({temperature: '72øF'});
  }
});
...
tool_results = [
    {"type": "tool_result", "tool_use_id": "toolu_01", "content": "72øF, Sunny"},
    {"type": "tool_result", "tool_use_id": "toolu_02", "content": "45øF, Cloudy"},

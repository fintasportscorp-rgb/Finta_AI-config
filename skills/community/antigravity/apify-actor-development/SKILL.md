...
```
.actor/
ÃÄÄ actor.json           # Actor config: name, version, env vars, runtime
ÃÄÄ input_schema.json    # Input validation & Console form definition
ÀÄÄ output_schema.json   # Output storage and display templates
src/
ÀÄÄ main.js/ts/py       # Actor entry point
storage/                # Local-only storage (NOT synced to Apify Console)
ÃÄÄ datasets/           # Output items (JSON objects)
ÃÄÄ key_value_stores/   # Files, config, INPUT
ÀÄÄ request_queues/     # Pending crawl requests

...
EVIDENCE: no-fabricate no-guess | ???:???/???  ???? | Read/Grep?? curl?? | "??"/"??"????test | guess=shame-wall

# FIVE LINES = each rule gets full attention
EVIDENCE:
  core: no-fabricate | no-guess | unsure=say-so
  banned: ???/???/???/??  ????
  proof: all-claims-need(data/line#/source) | Read/Grep?? | curl??
  hear-doubt: "??"/"??"  self-test(curl/benchmark)  ???user
...
```
# AI must infer: what does (???) modify? what does ?? apply to?
GATE-1: ???????????(???)(??????????????) | ??: signals????????=????

# AI reads labels directly: triggeractionexception. Zero ambiguity.
GATE-1 ??:
  trigger: new-task
  action: first-sentence="??????___"
  persist: ??????????????
  exception: signal=????  skip
  yields-to: GATE-3
```
...
????????schema | ??API/endpoint=???health-check.py??

# ANCHORED: label "new-api:" directly matches user saying "?? API"
MOAT:
  new-feature: ????schema/??/??
  new-api: ???health-check.py??(GATE-5)
```
...
Decomposition:
  ̀ TRIGGER:    "????"  new-task
  ̀ ACTION:     "???????"  first-sentence="??????___"
  ̀ DELETE:     "(???)"  human motivation, AI doesn't need this
  ̀ METADATA:   "(??????????????)"  persist: every-new-task
  ˀ EXCEPTION:  "??: signals????????=????"  exception: signal=????  skip
```
...
banned: ???/???/???/??  ????
hear-doubt: "??"/"??"  self-test(curl/benchmark)
self-serve: ???user(???)
compare: "??A?B?"  A/B????
```
...
```
GATE-1 (??: repeat task) vs GATE-3 (???: backup first)
...
GATE-4 (???: cite evidence) vs bug-close (????: write root cause)
...
EVIDENCE (no-guess) vs user says "????" (just do it)
 CONFLICT: should AI verify assumptions or execute immediately?
 RESOLUTION: signal "????" = user has decided, skip confirmation
```
...
like "???????". Adding `not-triggered: ?????` fixed it immediately.
...
6. Score each answer: ? full compliance, ?? partial, ? miss
...
T6: User says "??X?Y?" (does AI run comparison or just accept it?)
T7: User says "????" (does AI skip GATE-1 confirmation?)
...
```
action: first-sentence="??????___"     AI outputs Chinese
format: must-be-line-1                       structural constraint in English
banned: ???/???                         forbidden words stay in original language
```

**Why this works:** English label vocabulary (`trigger`, `action`, `exception`) maps directly
to concepts in every model's training data. Chinese grammar labels (????, ????, ????)
...
```xml
<gates label="???? | ???: gates>rules>rhythm | ???=STOP">
...gates here...
</gates>
...
MOAT:
  new-api: ???health-check.py??(GATE-5)     cross-ref, not duplicate
```

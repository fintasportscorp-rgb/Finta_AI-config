...
```
Account
ÀÄÄ Agent (AI persona - owns numbers, handles calls/SMS)
    ÃÄÄ Phone Number (attached to agent)
    ³   ÃÄÄ Call (inbound/outbound voice)
    ³   ³   ÀÄÄ Transcript (call recording text)
    ³   ÀÄÄ Message (SMS)
    ³       ÀÄÄ Conversation (threaded SMS exchange)
    ÀÄÄ Webhook (per-agent event delivery)

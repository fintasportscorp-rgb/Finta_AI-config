...
3. **Enhanced Measurement** - enable all, then review:
   ...
   - ?? Video engagement (disable if you'll track videos manually - avoid duplicates)
   - ?? File downloads (disable if you'll track these in GTM for better parameters)
...
GTM Container
ÃÄÄ Tags
³   ÃÄÄ GA4 Configuration (fires on all pages)
³   ÃÄÄ GA4 Event - [event_name] (one tag per event)
³   ÃÄÄ Google Ads Conversion (per conversion action)
³   ÀÄÄ Meta Pixel (if running Meta ads)
ÃÄÄ Triggers
³   ÃÄÄ All Pages
³   ÃÄÄ DOM Ready
³   ÃÄÄ Data Layer Event - [event_name]
³   ÀÄÄ Custom Element Click - [selector]
ÀÄÄ Variables
    ÃÄÄ Data Layer Variables (dlv - for each dL key)
    ÃÄÄ Constant - GA4 Measurement ID
    ÀÄÄ JavaScript Variables (computed values)
```

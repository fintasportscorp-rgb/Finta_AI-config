...
2. **Risk Assessment Update**
   ...
   ```
   ?? If any risk score >18 (Avoid threshold), STOP and initiate escalation to project sponsor before proceeding.

3. **Capacity Analysis**
   ```bash
   python3 scripts/resource_capacity_planner.py current_portfolio.json
   ```
   ?? If any team utilization >90% or <60%, flag for immediate reallocation discussion before step 4.

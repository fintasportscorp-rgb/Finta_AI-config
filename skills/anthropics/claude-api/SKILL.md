...
0. Are you deploying through Amazon Bedrock, Google Vertex AI, or Microsoft Foundry?
   ÀÄÄ Yes  Claude API (+ tool use for agents) - Managed Agents is 1P only.
   No  continue.

1. Single LLM call (classification, summarization, extraction, Q&A)
   ÀÄÄ Claude API - one request, one response

2. Do you want Anthropic to run the agent loop and host a per-session
   container where Claude executes tools (bash, file ops, code)?
   ÀÄÄ Yes  Managed Agents - server-managed sessions, persisted agent configs,
...
3. Workflow (multi-step, code-orchestrated, with your own tools)
   ÀÄÄ Claude API with tool use - you control the loop

4. Open-ended agent (model decides its own trajectory, your own tools, you host the compute)
   ÀÄÄ Claude API agentic loop (maximum flexibility)
```

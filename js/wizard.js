...
function generateFiles(ans, level, agentContents = {}, skillContents = {}, commandContents = {}) {
  ...
  // config folders that assistants treat specially.
  if (ans.loop_layer === 'yes' && window.LoopForgeTemplates) {
    const loopCfg  = deriveLoopConfig(ans, { name, stack, autonomy, tdd, neverDo, cmds, primaryTool, hasPy, hasTS, hasGo, hasRust });
    const built    = window.LoopForgeTemplates.buildLoopFiles(loopCfg);
    const loopRoot = `loop/${loopCfg.name}/`;
    Object.entries(built.files).forEach(([p, content]) => { F[loopRoot + p] = content; });
    F[mainFile] += [
      ``, ``, `## Loop Execution Layer`,
      `This project ships a loop-engineering contract in \`${loopRoot}\` (generated by Finta Loop Forge).`,
      `It defines HOW work gets finished: a verified loop with guardrails enforced in code,`,
      `two-mode verification (deterministic/soft), on-disk memory and human escalation.`,
      `- Read \`${loopRoot}AGENT.md\` before long-running or multi-step tasks and respect its scope.`,
      `- Validate the bundle: \`python3 ${loopRoot}script/validate-structure.py && python3 ${loopRoot}script/validate-coherence.py\``,
      `- Run a loop turn: \`python3 ${loopRoot}script/loop_runner.py "task"\` (exit 0 resolved · 2 human needed · 3 clarify).`,
      `- Fine-tune it anytime in Loop Forge: https://config.fintalab.com/loopforge.html`,
    ].join('\n');
  }
...
function deriveLoopConfig(ans, d) {
  const slugify = s => String(s).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 48) || 'my-project';

  // Autonomy → loop profile + strictness
  const profile = d.autonomy === 'conservative' ? 'human_validated'
                : d.autonomy === 'autonomous'   ? 'proactive'
                : 'goal_based';
  const maxIter = d.autonomy === 'conservative' ? 10 : d.autonomy === 'autonomous' ? 30 : 20;
  const doubt   = d.autonomy === 'conservative' ? 5  : 10;
  ...
  criteria.push({ label: 'Output matches the agreed spec', mode: 'soft',
                  check: 'Compare result to the spec validated with the user (memory.md)', weight: 2 });

  const forbid = (ans.never_do || '').split('\n').map(l => l.trim()).filter(Boolean);

  // Mission from the first documented spec, if any (specs = [{title, description, …}])
  const firstSpec = [].concat(ans.specs || []).find(s => s && (s.title || s.description));
  const goal = firstSpec
    ? [firstSpec.title, firstSpec.description].filter(Boolean).join(' — ')
    : `Advance ${d.name} tasks to a verified done state: tests green, spec respected, nothing outside scope touched.`;

  return {
    name: slugify(d.name) + '-loop',
    runtime: d.primaryTool === 'claude_code' ? 'claude' : 'any',
    goal,
    scope: ['read the repo', 'run tests and linters', 'edit source and test files inside the project'],
    forbid: forbid.length ? forbid : ['modify the project scope itself', 'push to remote or deploy', 'delete files outside the project'],
    profile,
    pattern: 'single',
    subs: [],
    criteria,
    ...
    guard: {
      ...
      stagnation_rounds: 3,
      doubt_threshold_pct: doubt,
      retest_max_attempts: 3,
...
<div class="boot-box" style="display:flex;flex-wrap:wrap;align-items:center;gap:.9rem;justify-content:space-between">
  <label style="display:flex;align-items:flex-start;gap:.7rem;cursor:pointer;flex:1;min-width:260px">
    <input type="checkbox" ${S.ans.loop_layer === 'yes' ? 'checked' : ''} onchange="toggleLoopLayer(this)" style="margin-top:.3rem;accent-color:var(--grn);width:16px;height:16px;cursor:pointer">
    <span>
      <strong style="color:var(--grn)">${t('loop_layer_title')}</strong><br>
      <span style="font-size:.8rem;color:var(--grn3)">${t('loop_layer_desc')}</span>
    </span>
  </label>
...
function buildBootPromptText(ans, name) {
  const isImprove = ans.objectif === 'improve_existing';
  const loopLine = ans.loop_layer === 'yes'
    ? `\n# The zip includes loop/ — an execution contract with guardrails.\n# Also run: python3 loop/*/script/validate-structure.py`
    : '';
  if (!isImprove) {
    return `unzip ${name}-ai-setup.zip\n# Then tell your assistant:\n# "Read and execute prompts/bootstrap-init.md"${loopLine}`;
  }
  return [
